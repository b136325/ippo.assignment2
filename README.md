...
The application enables navigation between walls (and images) within a room, along with navigation between rooms.
In addition, standalone items can be 'picked up' from a room or 'put down' into a room. The 
...
Please note that due to bug within IntelliJ I was unable to access the `Scene Builer` facility. Consequently, not only were the .fxml views were generated by hand, but also - unfortunately - the layout may be rather crude in places.
Further information about the IntelliJ can be found at the following link: `https://intellij-support.jetbrains.com/hc/en-us/community/posts/360004371099-Scene-Builder-does-not-work-anymore-after-update-to-2019-2-version`.
